...
export function SpotlightCard({ to, className = "", glowColor = "#7cb8d8", children }: any) {
  ...
  return (
    <Link
      ...
      className={`group relative overflow-hidden rounded-2xl bg-white/95 text-foreground border border-slate-100 shadow-sm transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 focus:outline-none ${className}`}
      style={{
    ...
    >
      ...
      {/* Content */}
      <div className="relative z-10 w-full h-full flex flex-col p-6 md:p-8">{children}</div>
    </Link>
  );
}
