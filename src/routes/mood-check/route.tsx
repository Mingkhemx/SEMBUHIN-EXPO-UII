...
export const Route = createFileRoute('/mood-check')({
  head: () => ({
    meta: [
      ...
      { name: 'description', content: 'Cek mood via kamera menggunakan Sembuhin Vision AI.' },
    ],
  }),
...
interface MoodDetection {
  emotion: string; pct: number; icon: typeof Smile
  color: string; description: string; rawKey: string
}
interface MoodResult {
  primaryMood: MoodLevel; confidence: number; description: string
...
interface AIEmotionResult {
  faceDetected: boolean; emotions: Record<string, number>
  primaryEmotion: string; confidence: number; geminiDescription: string
}
...
const EMOTION_MAP: Record<string, Omit<MoodDetection, 'pct'>> = {
  ...
  fearful:  { emotion: 'Cemas',    icon: AlertTriangle, color: 'text-amber-600',   description: 'Tanda ketegangan terdeteksi',    rawKey: 'fearful'  },
...
const MOOD_CONFIG: Record<MoodLevel, { label: string; color: string; bgColor: string; emoji: string; desc: string }> = {
  happy:   { label:'Senang',  color:'text-emerald-700', bgColor:'bg-emerald-50 border-emerald-200', emoji:'😊', desc:'Anda terlihat dalam kondisi emosional yang baik!' },
  calm:    { label:'Tenang',  color:'text-sky-700',     bgColor:'bg-sky-50 border-sky-200',         emoji:'😌', desc:'Anda tampak tenang dan rileks.' },
  neutral: { label:'Netral',  color:'text-slate-700',   bgColor:'bg-slate-50 border-slate-200',     emoji:'😐', desc:'Ekspresi Anda netral saat ini.' },
  anxious: { label:'Cemas',   color:'text-amber-700',   bgColor:'bg-amber-50 border-amber-200',     emoji:'😰', desc:'Terdeteksi tanda-tanda kecemasan.' },
  sad:     { label:'Sedih',   color:'text-indigo-700',  bgColor:'bg-indigo-50 border-indigo-200',   emoji:'😢', desc:'Anda tampak sedih.' },
  angry:   { label:'Marah',   color:'text-red-700',     bgColor:'bg-red-50 border-red-200',         emoji:'😠', desc:'Terdeteksi ekspresi kemarahan.' },
  tired:   { label:'Lelah',   color:'text-violet-700',  bgColor:'bg-violet-50 border-violet-200',   emoji:'😫', desc:'Anda tampak kelelahan.' },
}

const MOOD_ACTIVITIES: Record<MoodLevel, MoodResult['activities']> = {
  happy:   [
    { title:'Jalan Kaki 15 Menit',   desc:'Pertahankan mood positif',              icon:Sun,      duration:'15 min' },
    { title:'Journaling Syukur',     desc:'Tulis 3 hal yang Anda syukuri',          icon:BookOpen, duration:'10 min' },
    { title:'Musik Favorit',         desc:'Dengarkan playlist favorit Anda',        icon:Music,    duration:'20 min' },
    { title:'Hubungi Teman',         desc:'Ceritakan kabar baik ke orang terdekat', icon:Users,    duration:'15 min' },
  ],
  calm:    [
    { title:'Meditasi Mindfulness',  desc:'Pertahankan ketenangan',                 icon:Brain,    duration:'10 min' },
    { title:'Journaling',            desc:'Refleksi pikiran saat tenang',            icon:BookOpen, duration:'15 min' },
  ...
  neutral: [
    { title:'Jalan Santai',          desc:'Tingkatkan mood dengan gerakan ringan',   icon:Sun,      duration:'15 min' },
    { title:'Baca Buku',             desc:'Isi waktu dengan bacaan menarik',         icon:BookOpen, duration:'20 min' },
    { title:'Hubungi Sahabat',       desc:'Obrolan ringan bisa meningkatkan mood',   icon:Users,    duration:'15 min' },
    { title:'Musik Upbeat',          desc:'Playlist ceria untuk membangkitkan semangat',icon:Music, duration:'20 min' },
  ],
  anxious: [
    { title:'Pernapasan 4-7-8',      desc:'Tarik 4 detik, tahan 7, hembuskan 8',    icon:Wind,     duration:'5 min'  },
    { title:'Grounding 5-4-3-2-1',   desc:'Fokus ke 5 benda, 4 suara, 3 sentuhan',  icon:Brain,    duration:'3 min'  },
    { title:'Jalan di Alam',         desc:'Alam terbukti menurunkan kortisol',       icon:Sun,      duration:'20 min' },
    { title:'Teh Chamomile',         desc:'Herbal untuk menenangkan sistem saraf',   icon:Heart,    duration:'10 min' },
  ],
  sad:     [
    { title:'Jalan di Alam',         desc:'Alam meningkatkan mood secara alami',     icon:Sun,      duration:'20 min' },
    { title:'Hubungi Orang Terdekat',desc:'Berbagi meringankan beban emosional',     icon:Users,    duration:'15 min' },
    { title:'Journaling Perasaan',   desc:'Tulis apa yang Anda rasakan',             icon:BookOpen, duration:'15 min' },
    { title:'Musik yang Disukai',    desc:'Musik menemani dan mengangkat perasaan',  icon:Music,    duration:'20 min' },
  ],
  angry:   [
    { title:'Napas Kotak',           desc:'Tarik 4s, tahan 4s, hembuskan 4s',       icon:Wind,     duration:'5 min'  },
    { title:'Olahraga Ringan',       desc:'Lepaskan energi negatif lewat fisik',     icon:Sparkles, duration:'15 min' },
    { title:'Tulis Uneg-uneg',       desc:'Tuliskan lalu buang',                     icon:BookOpen, duration:'10 min' },
    { title:'Jalan Cepat',           desc:'Aerobik menurunkan hormon stres',         icon:Sun,      duration:'15 min' },
  ],
  tired:   [
    { title:'Power Nap',             desc:'Tidur 20 menit untuk recharge',           icon:Heart,    duration:'20 min' },
    { title:'Stretching Ringan',     desc:'Regangkan otot leher & punggung',         icon:Sparkles, duration:'10 min' },
    { title:'Air Putih & Snack',     desc:'Dehidrasi sering menyebabkan lelah',      icon:Wind,     duration:'5 min'  },
    { title:'Musik Energik',         desc:'Upbeat membantu melawan ngantuk',         icon:Music,    duration:'15 min' },
  ],
}

const MOOD_RECOMMENDATIONS: Record<MoodLevel, string> = {
  happy:   'Mood Anda sedang bagus! Manfaatkan energi positif ini.',
  calm:    'Ketenangan Anda sangat berharga. Gunakan untuk fokus.',
  neutral: 'Mood stabil. Waktu yang baik untuk aktivitas baru.',
  anxious: 'Terdeteksi kecemasan. Prioritaskan teknik relaksasi.',
  sad:     'Anda boleh merasakan kesedihan. Cari dukungan orang terdekat.',
  angry:   'Kemarahan valid. Salurkan dengan cara yang konstruktif.',
  tired:   'Tubuh meminta istirahat. Dengarkan sinyal itu.',
}

interface MoodRecoDetail {
  quote: string; quoteAuthor: string; context: string
  accentColor: string; accentText: string; accentBorder: string
  priorityLabel: string; tips: string[]
}
const MOOD_RECO_DETAIL: Record<MoodLevel, MoodRecoDetail> = {
  happy:   { quote:'Kebahagiaan bukan tujuan — ia adalah cara kita menjalani perjalanan.', quoteAuthor:'Margaret Lee Runbeck', context:'Pertahankan energi positifmu', accentColor:'bg-emerald-50', accentText:'text-emerald-800', accentBorder:'border-emerald-200', priorityLabel:'✨ Rayakan momen ini', tips:['Bagikan kebahagiaan ke orang-orang terdekat','Catat hal baik hari ini'] },
  calm:    { quote:'Ketenangan pikiran adalah kebahagiaan yang sesungguhnya.', quoteAuthor:'Epictetus', context:'Manfaatkan ketenanganmu', accentColor:'bg-sky-50', accentText:'text-sky-800', accentBorder:'border-sky-200', priorityLabel:'🎯 Waktu terbaik untuk fokus', tips:['Kerjakan tugas penting sekarang','Meditasi singkat memperkuat ketenangan'] },
  neutral: { quote:'Setiap hari adalah kesempatan baru untuk menjadi lebih baik.', quoteAuthor:'Anonim', context:'Beri dirimu sedikit dorongan', accentColor:'bg-slate-50', accentText:'text-slate-800', accentBorder:'border-slate-200', priorityLabel:'💡 Bangkitkan semangatmu', tips:['Coba satu hal baru hari ini','Gerakan fisik ringan terbukti meningkatkan mood'] },
  anxious: { quote:'Kamu sudah melewati 100% hari-hari terberatmu.', quoteAuthor:'Anonim', context:'Tenangkan dirimu dulu', accentColor:'bg-amber-50', accentText:'text-amber-800', accentBorder:'border-amber-200', priorityLabel:'🫁 Mulai dari napas dulu', tips:['Tarik napas 4 detik, tahan 4, hembuskan 4','Batasi media sosial sementara'] },
  sad:     { quote:'Tidak apa-apa untuk tidak baik-baik saja.', quoteAuthor:'Anonim', context:'Peluk perasaanmu', accentColor:'bg-indigo-50', accentText:'text-indigo-800', accentBorder:'border-indigo-200', priorityLabel:'💙 Yang kamu butuhkan sekarang', tips:['Cerita ke orang yang kamu percaya','Beri dirimu izin merasakannya'] },
  angry:   { quote:'Kemarahan yang dikelola bijak bisa jadi bahan bakar perubahan.', quoteAuthor:'Anonim', context:'Salurkan energimu', accentColor:'bg-red-50', accentText:'text-red-800', accentBorder:'border-red-200', priorityLabel:'🔥 Lepaskan dulu', tips:['Jangan buat keputusan saat masih marah','Gerak fisik meredakan amarah tercepat'] },
  tired:   { quote:'Istirahat bukan kelemahan. Istirahat adalah bagian dari perjuangan.', quoteAuthor:'Anonim', context:'Tubuhmu butuh pemulihan', accentColor:'bg-violet-50', accentText:'text-violet-800', accentBorder:'border-violet-200', priorityLabel:'😴 Izinkan dirimu beristirahat', tips:['Power nap 20 menit lebih efektif dari kafein','Jauhkan layar 30 menit sebelum tidur'] },
}
...
function buildMoodResult(emotions: Record<string,number>, confidence: number, desc?: string): MoodResult {
  const detections: MoodDetection[] = Object.entries(emotions).filter(([k])=>EMOTION_MAP[k]).map(([k,v])=>({...EMOTION_MAP[k],pct:Math.round(v*100)})).sort((a,b)=>b.pct-a.pct)
  const top = detections[0]
  const primaryMood = emotionToMoodLevel(top?.rawKey ?? 'neutral')
  return { primaryMood, confidence, description:desc??MOOD_CONFIG[primaryMood].desc, recommendation:MOOD_RECOMMENDATIONS[primaryMood], activities:MOOD_ACTIVITIES[primaryMood], detections }
}
...
function simulateEmotions(frame: number): Record<string,number> {
  const keys = ['happy','calm','neutral','fearful','sad','angry','disgusted','surprised']
  const result: Record<string,number> = {}
  for (const k of keys) {
    const base = k==='neutral' ? 0.35 : 0.08
    result[k] = Math.max(0, base + (Math.random()-0.5)*0.25)
  }
...
function MoodCheckPage() {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      const { data } = await supabase.from('mood_history').select('id,created_at,mood,confidence').eq('user_id',user.id).order('created_at',{ascending:false})
  ...
  const saveResult = useCallback(async (r: MoodResult) => {
    if (!user) return
  ...
  const stopCamera = useCallback(() => {
    ...
    setCameraActive(false); setScanning(false); setScanProgress(0)
    setFaceFound(false); setLiveDetections([]); setAutoCountdown(null)
  }, [])

  const startScan = useCallback(async () => {
    const b64 = captureFrame()
    if (!b64) return
    setScanning(true); setScanProgress(0); setFaceFound(false); setLiveDetections([])
    aiResultRef.current = analyzeWithOpenRouter(b64)

    const FRAMES=30, MS=167
    frameRef.current=0
    const tick = async () => {
      const f = frameRef.current
      setScanProgress(Math.min(Math.round((f/FRAMES)*100), f<FRAMES?99:100))
      const sim = simulateEmotions(f)
      const dets: MoodDetection[] = Object.entries(sim).filter(([k])=>EMOTION_MAP[k]).map(([k,v])=>({...EMOTION_MAP[k],pct:Math.round(v*100)})).sort((a,b)=>b.pct-a.pct)
      setLiveDetections(dets); setFaceFound(true)
      frameRef.current++
      if (frameRef.current < FRAMES) { setTimeout(tick, MS); return }
      // Done — await AI
      setScanProgress(100)
      const ai = await aiResultRef.current
      streamRef.current?.getTracks().forEach(t=>t.stop()); streamRef.current=null
      setCameraActive(false); setScanning(false)
      if (!ai) { setError('Analisis gagal. Pastikan koneksi internet stabil.'); return }
      if (!ai.faceDetected) { setFaceFound(false); setScanProgress(0); setError('Tidak ada wajah terdeteksi. Pastikan wajah Anda terlihat jelas, lalu coba lagi.'); return }
      const finalDets: MoodDetection[] = Object.entries(ai.emotions).filter(([k])=>EMOTION_MAP[k]).map(([k,v])=>({...EMOTION_MAP[k],pct:Math.round(v*100)})).sort((a,b)=>b.pct-a.pct)
      setLiveDetections(finalDets)
      const result = buildMoodResult(ai.emotions, ai.confidence, ai.geminiDescription)
      setMoodResult(result); setGeminiUsed(true); setViewMode('result'); saveResult(result)
    }
    setTimeout(tick, MS)
  }, [saveResult])
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      let c=3; setAutoCountdown(c)
      const tick = () => {
        c-=1
        if (c>0) { setAutoCountdown(c); countdownRef.current=setTimeout(tick,1000) }
        else { setAutoCountdown(0); countdownRef.current=setTimeout(()=>{ setAutoCountdown(null); startScan() },300) }
      }
      countdownRef.current = setTimeout(tick, 1000)
    } catch { setError('Gagal mengakses kamera! Izinkan akses kamera di browser.') }
  }

  const resetAll = () => { stopCamera(); setMoodResult(null); setGeminiUsed(false); setError(null); setViewMode('camera') }

  const scanLabel = scanProgress<25?'Mendeteksi wajah...':scanProgress<50?'Membaca ekspresi...':scanProgress<75?'Menganalisis emosi...':scanProgress<100?'Menyempurnakan hasil...':'✓ Analisis selesai!'
  const CIRC_R=48, CIRC_C=2*Math.PI*CIRC_R
      ...
      <div className="relative z-10 min-h-screen">
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-16 sm:py-20 space-y-14">
          ...
          <div className="max-w-3xl mx-auto">
            ...
            {/* ── CAMERA VIEW ── */}
            {viewMode === 'camera' && (
              <motion.div key="camera" initial={{opacity:0}} animate={{opacity:1}} className="space-y-6">
                {/* Error banner */}
                {error && (
                  <motion.div initial={{opacity:0,y:-8}} animate={{opacity:1,y:0}}
                    className={cn('flex items-start gap-3 rounded-2xl border p-5', error.includes('wajah')?'bg-amber-50 border-amber-200':'bg-rose-50 border-rose-200')}
                  >
                    <div className={cn('flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-2xl', error.includes('wajah')?'bg-amber-100':'bg-rose-100')}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className={cn('text-xs leading-relaxed', error.includes('wajah')?'text-amber-700':'text-rose-700')}>{error}</p>
                    ...
                    {!error.includes('wajah') && (
                      <button onClick={()=>setError(null)} className="shrink-0"><X className="h-4 w-4 text-rose-400" /></button>
                    )}
                  </motion.div>
                )}
                  ...
                  <div className="space-y-0">
                    <div className="relative rounded-t-2xl overflow-hidden shadow-2xl bg-black" style={{aspectRatio:'4/3'}}>
                      ...
                      {scanning && (
                        <div className="absolute inset-0 pointer-events-none overflow-hidden">
                          <motion.div className="absolute left-0 right-0 h-px bg-gradient-to-r from-transparent via-cyan-400 to-transparent"
                            ...
                            animate={{top:['5%','95%','5%']}} transition={{duration:2.4,repeat:Infinity,ease:'linear'}} />
                        </div>
                      )}
                      {!scanning && (
                        <div className="absolute inset-x-0 bottom-20 flex justify-center pointer-events-none">
                          {autoCountdown!==null && autoCountdown>0 ? (
                            <motion.div key={autoCountdown} initial={{scale:1.4,opacity:0}} animate={{scale:1,opacity:1}}
                              className="flex items-center gap-3 bg-black/55 backdrop-blur-sm text-white font-bold px-6 py-3 rounded-2xl">
                              <span className="text-3xl font-black text-cyan-300">{autoCountdown}</span>
                              <span className="text-sm">Scan dimulai dalam...</span>
                            </motion.div>
                          ) : (
                            <div className="flex items-center gap-2 bg-cyan-500/80 backdrop-blur-sm text-white text-xs font-semibold px-4 py-2 rounded-full">
                              <div className="h-2 w-2 rounded-full bg-white animate-pulse" />Kamera aktif
                            </div>
                          )}
                        </div>
                      )}
                        ...
                        <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 flex flex-col items-center gap-3 px-8 pointer-events-none">
                          <div className="relative flex items-center justify-center">
                            <svg className="w-32 h-32 -rotate-90 drop-shadow-xl" viewBox="0 0 112 112">
                              ...
                              <motion.circle cx="56" cy="56" r={CIRC_R} fill="none" stroke={faceFound?'#34d399':'#22d3ee'}
                                strokeWidth="7" strokeLinecap="round" strokeDasharray={`${CIRC_C}`}
                                animate={{strokeDashoffset:CIRC_C*(1-scanProgress/100)}} transition={{duration:0.4,ease:'easeOut'}} />
                            </svg>
                            <div className="absolute flex flex-col items-center">
                              {scanProgress<100 ? (
                                <><motion.span key={scanProgress} initial={{scale:0.75,opacity:0}} animate={{scale:1,opacity:1}}
                                    className="text-4xl font-black text-white drop-shadow-lg tabular-nums">{scanProgress}%</motion.span>
                                  <span className="text-[10px] text-cyan-200 font-semibold mt-1">{faceFound?'Wajah terdeteksi ✓':'Memindai...'}</span>
                                </>
                              ) : (
                                <motion.div initial={{scale:0}} animate={{scale:1}} transition={{type:'spring',stiffness:260}} className="flex flex-col items-center">
                          ...
                          <div className="w-full max-w-xs">
                            <div className="h-2.5 rounded-full bg-white/20 overflow-hidden">
                              <motion.div className={cn('h-full rounded-full bg-gradient-to-r',faceFound?'from-emerald-400 to-teal-300':'from-cyan-400 to-sky-300')}
                                animate={{width:`${scanProgress}%`}} transition={{duration:0.4,ease:'easeOut'}} />
                            </div>
                          </div>
                      ...
                      <div className="absolute top-3 right-3 z-50">
                        <button onClick={stopCamera} className="flex h-9 w-9 items-center justify-center rounded-full bg-black/40 backdrop-blur-sm text-white hover:bg-black/60 transition-all border border-white/20">
                          <X className="h-4 w-4" />
                        </button>
                      </div>
                    </div>
                        ...
                        <div className="flex items-center gap-1.5">
                          {scanning ? (
                            <><div className="h-2 w-2 rounded-full bg-cyan-500 animate-pulse" />
                              <span className="text-[10px] font-semibold text-cyan-600">{Math.round(scanProgress*30/100)} frame dianalisis</span></>
                      ...
                      <div className="p-4 space-y-2 min-h-[180px]">
                        ...
                        <AnimatePresence>
                          {liveDetections.map((det,i)=>{
                            const Icon=det.icon
                            return (
                              <motion.div key={det.emotion} initial={{opacity:0,x:-12}} animate={{opacity:1,x:0}} transition={{duration:0.3}}
                                className={cn('flex items-center gap-3 p-3 rounded-xl border',i===0?'bg-cyan-50 border-cyan-200 shadow-sm':'bg-slate-50 border-slate-100')}>
                                <div className="relative flex h-12 w-12 shrink-0 items-center justify-center">
                                  <svg className="h-12 w-12 -rotate-90" viewBox="0 0 48 48">
                                    <circle cx="24" cy="24" r="20" fill="none" stroke="#f1f5f9" strokeWidth="4" />
                                    <motion.circle cx="24" cy="24" r="20" fill="none" stroke={i===0?'#06b6d4':'#94a3b8'}
                                      strokeWidth="4" strokeLinecap="round" strokeDasharray={`${2*Math.PI*20}`}
                                      animate={{strokeDashoffset:2*Math.PI*20*(1-det.pct/100)}} transition={{duration:0.4}} />
                                  </svg>
                      ...
                      {!scanning && autoCountdown===null && liveDetections.length===0 && (
                        <div className="px-4 pb-4">
                          <button onClick={startScan}
                            className="w-full flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-cyan-600 to-sky-600 text-white px-6 py-3.5 text-sm font-bold hover:from-cyan-700 hover:to-sky-700 shadow-lg transition-all hover:scale-[1.01]">
              ...
              <motion.div key="result" initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{duration:0.5}} className="space-y-6">
                    ...
                    <div className="flex items-center gap-5">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          <p className={cn('text-lg font-bold',MOOD_CONFIG[moodResult.primaryMood].color)}>Mood: {MOOD_CONFIG[moodResult.primaryMood].label}</p>
                          <span className="text-xs text-slate-500">• Akurasi {moodResult.confidence}%</span>
                          {geminiUsed && <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-violet-700 bg-violet-100 border border-violet-200 px-2 py-0.5 rounded-full"><Sparkles className="h-3 w-3" /> Sembuhin Vision</span>}
                        </div>
                ...
                <div className="rounded-2xl bg-white border border-white/60 shadow-lg p-6">
                  ...
                  <div className="h-3 rounded-full bg-slate-100 overflow-hidden">
                    <motion.div initial={{width:0}} animate={{width:`${moodResult.confidence}%`}} transition={{duration:1.1,delay:0.1,ease:'easeOut'}}
                      ...
                      return (
                        <div key={det.emotion} className={cn('flex items-center gap-3 p-3 rounded-xl border',i===0?'bg-cyan-50 border-cyan-200':'bg-slate-50 border-slate-100')}>
                          ...
                          <div className="flex-1 h-2 rounded-full bg-slate-200 overflow-hidden">
                            <motion.div initial={{width:0}} animate={{width:`${det.pct}%`}} transition={{duration:0.8,delay:i*0.1}}
                ...
                <div className="rounded-2xl bg-white border border-white/60 shadow-lg overflow-hidden">
                  <div className={cn('px-6 pt-6 pb-4',MOOD_RECO_DETAIL[moodResult.primaryMood].accentColor)}>
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className={cn('ml-auto text-[10px] font-bold px-2 py-0.5 rounded-full border',MOOD_RECO_DETAIL[moodResult.primaryMood].accentText,MOOD_RECO_DETAIL[moodResult.primaryMood].accentBorder,MOOD_RECO_DETAIL[moodResult.primaryMood].accentColor)}>
                        {MOOD_CONFIG[moodResult.primaryMood].emoji} {MOOD_CONFIG[moodResult.primaryMood].label}
                      </span>
                    </div>
                    <div className={cn('rounded-xl border p-4 mb-3',MOOD_RECO_DETAIL[moodResult.primaryMood].accentBorder,'bg-white/70')}>
                      <p className={cn('text-sm font-medium italic leading-relaxed',MOOD_RECO_DETAIL[moodResult.primaryMood].accentText)}>
                        "{MOOD_RECO_DETAIL[moodResult.primaryMood].quote}"
                      </p>
                      <p className="text-[11px] text-slate-400 mt-1.5 text-right">— {MOOD_RECO_DETAIL[moodResult.primaryMood].quoteAuthor}</p>
                    </div>
                    <p className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider mb-1">{MOOD_RECO_DETAIL[moodResult.primaryMood].context}</p>
                    <p className="text-sm text-slate-700 leading-relaxed">{moodResult.recommendation}</p>
                      ...
                      {MOOD_RECO_DETAIL[moodResult.primaryMood].tips.map((tip,i)=>(
                        <div key={i} className="flex items-start gap-2">
                          <div className={cn('mt-1 h-1.5 w-1.5 rounded-full shrink-0',
                            moodResult.primaryMood==='happy'?'bg-emerald-400':moodResult.primaryMood==='calm'?'bg-sky-400':
                            moodResult.primaryMood==='neutral'?'bg-slate-400':moodResult.primaryMood==='anxious'?'bg-amber-400':
                            moodResult.primaryMood==='sad'?'bg-indigo-400':moodResult.primaryMood==='angry'?'bg-red-400':'bg-violet-400')} />
                  ...
                  <div className="px-6 pb-6 pt-4">
                    <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3">{MOOD_RECO_DETAIL[moodResult.primaryMood].priorityLabel}</p>
                    <div className="grid sm:grid-cols-2 gap-3">
                      {moodResult.activities.map((act,i)=>{
                        const Icon=act.icon
                        return (
                          <motion.div key={act.title} initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} transition={{delay:i*0.08}}
                            className={cn('flex items-start gap-3 p-3 rounded-xl border transition-all hover:shadow-md',
                              i===0?cn(MOOD_RECO_DETAIL[moodResult.primaryMood].accentColor,MOOD_RECO_DETAIL[moodResult.primaryMood].accentBorder):'bg-slate-50 border-slate-100')}>
                            <div className={cn('flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border',i===0?'bg-white border-white/80 shadow-sm':'bg-white border-slate-200')}>
                              <Icon className={cn('h-4 w-4',i===0?MOOD_RECO_DETAIL[moodResult.primaryMood].accentText:'text-slate-500')} />
                            </div>
                            <div className="min-w-0">
                              <div className="flex items-center gap-1.5 flex-wrap">
                                <p className="text-xs font-bold text-slate-800">{act.title}</p>
                                {i===0 && <span className={cn('text-[9px] font-bold px-1.5 py-0.5 rounded-full border',MOOD_RECO_DETAIL[moodResult.primaryMood].accentText,MOOD_RECO_DETAIL[moodResult.primaryMood].accentBorder,'bg-white/60')}>#1</span>}
                              </div>
                              <p className="text-[10px] text-slate-500 mt-0.5 leading-relaxed">{act.desc}</p>
                              <p className={cn('text-[10px] font-bold mt-1',i===0?MOOD_RECO_DETAIL[moodResult.primaryMood].accentText:'text-slate-400')}>⏱ {act.duration}</p>
                            </div>
                          </motion.div>
                        )
                      })}
                ...
                <div className="flex gap-3 rounded-xl bg-white/80 border border-slate-200/80 p-4 shadow-sm">
                  ...
                  <p className="text-xs text-slate-500 leading-relaxed">Mood check bersifat indikatif dan bukan pengganti evaluasi psikologis profesional.</p>
                </div>
              </motion.div>
            )}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-slate-400">{moodHistory.length>0?`${moodHistory.length} sesi tercatat`:'Belum ada sesi'}</p>
                    </div>
                  </div>
                  {moodHistory.length>0 && <span className="text-[10px] font-semibold text-cyan-600 bg-cyan-50 border border-cyan-100 px-2.5 py-1 rounded-full">Sembuhin Vision 1.5</span>}
                </div>
                        ...
                        <motion.div key={entry.id} initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} transition={{delay:i*0.05}}
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                              ...
                              <span className={cn('text-[10px] font-semibold px-2 py-0.5 rounded-full border',conf.bgColor,conf.color)}>{conf.label}</span>
                            </div>
                            <p className="text-xs text-slate-400 flex items-center gap-1">
                              <Clock className="h-3 w-3 shrink-0" />{formatDate(entry.created_at)}
                            </p>
                          </div>
                          <div className="hidden sm:flex flex-col items-end gap-1 shrink-0 w-24">
                            ...
                            <div className="w-full h-1.5 rounded-full bg-slate-100 overflow-hidden">
                              <div className={cn('h-full rounded-full',
                                entry.mood==='happy'?'bg-emerald-400':entry.mood==='calm'?'bg-sky-400':entry.mood==='neutral'?'bg-slate-400':
                                entry.mood==='anxious'?'bg-amber-400':entry.mood==='sad'?'bg-indigo-400':entry.mood==='angry'?'bg-red-400':'bg-violet-400'
